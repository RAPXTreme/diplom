...
def add_book_db(db: Session, author: str, description: str, publish_date: date, title: str, user_id: int,price: str, photo_path: str) -> Book:
    new_book = Book(author=author, description=description, publish_date=publish_date, title=title, user_id=user_id, price=price, photo_path=f'/static/{photo_path}')
    db.add(new_book)
    db.commit()
    db.refresh(new_book)
    return new_book
